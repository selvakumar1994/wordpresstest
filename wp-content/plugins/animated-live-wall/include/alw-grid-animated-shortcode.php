		...
		<div id="ri-grid-<?php echo $alw_id; ?>" class="ri-grid ri-grid-size-2">
			...
			if(isset($alw_get_settings['image-ids']) && count($alw_get_settings['image-ids']) > 0) {
				...
				foreach($alw_get_settings['image-ids'] as $attachment_id) {
					...
					//$image_type = $alw_get_settings['slide-type'][$attachment_id];
					...
					//set thumbnail size
					if($alw_grid_thumb_size == "thumbnail") { $thumbnail_url = $thumbnail[0]; }
					if($alw_grid_thumb_size == "medium") { $thumbnail_url = $medium[0]; }
					if($alw_grid_thumb_size == "large") { $thumbnail_url = $large[0]; }
					if($alw_grid_thumb_size == "full") { $thumbnail_url = $full[0]; }
					if($alw_grid_thumb_size == "custum_500x500") { $thumbnail_url = $custum_500x500[0]; }
					if($alw_grid_thumb_size == "custum_800x800") { $thumbnail_url = $custum_800x800[0]; }
					//lightbox
					if($alw_lightbox_thumb_size == "thumbnail") { $thumbnail_url_lightbox = $thumbnail[0]; }
					if($alw_lightbox_thumb_size == "medium") { $thumbnail_url_lightbox = $medium[0]; }
					if($alw_lightbox_thumb_size == "large") { $thumbnail_url_lightbox = $large[0]; }
					if($alw_lightbox_thumb_size == "full") { $thumbnail_url_lightbox = $full[0]; }
					if($alw_lightbox_thumb_size == "custum_500x500") { $thumbnail_url_lightbox = $custum_500x500[0]; }
					if($alw_lightbox_thumb_size == "custum_800x800") { $thumbnail_url_lightbox = $custum_800x800[0]; }
					?>
					<li class="brick">
						<?php // get video id from youtube vemio 
						if (strpos($image_link_url, 'youtube') !== false) {
							...
							$lightboxop = 'pw-lightbox'; $link_url = $thumbnail_url; $video_icon = ''; $vedio_class = '';
						} else {
							$lightboxop = ''; $video_icon = 'fas fa-link'; $link_url = $image_link_url; $vedio_class = '';
						}
						?>
						<a href="<?php echo esc_url($link_url); ?>" class="<?php echo $lightboxop; ?> <?php echo $vedio_class; ?>" rel="next img-wrapper" target="<?php echo $alw_img_redirection; ?>">
							<img src="<?php echo $thumbnail_url; ?>" width="100%" alt="<?php echo $title; ?>"/>
							<i class="alw-icon <?php echo $video_icon; ?>"></i>
						</a>
					</li>
					<?php
					$no++;
