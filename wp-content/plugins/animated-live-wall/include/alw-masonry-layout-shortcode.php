		...
		if(isset($alw_get_settings['image-ids']) && count($alw_get_settings['image-ids']) > 0) {
			$alw_total_images = count($alw_get_settings['image-ids']);
				...
				foreach($alw_get_settings['image-ids'] as $attachment_id) {
					...
					//set thumbnail size
					if($alw_thumb_size == "thumbnail") { $thumbnail_url = $thumbnail[0]; }
					if($alw_thumb_size == "medium") { $thumbnail_url = $medium[0]; }
					if($alw_thumb_size == "large") { $thumbnail_url = $large[0]; }
					if($alw_thumb_size == "full") { $thumbnail_url = $full[0]; }
					...
					<div class="brick">
						...
						?>
						<a href="<?php echo esc_url($link_url); ?>" class="<?php echo $lightboxop; ?> <?php echo $vedio_class; ?>" target="<?php echo $alw_maso_img_redirection; ?>">
						...
						<!--<a href="<?php //echo $image_link_url; ?>" class="pw-more">View more</a>-->
					</div>
					<?php
					$count++;					
